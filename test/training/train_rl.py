...
if __name__ == '__main__':
    ...
    eval_env = SubprocVecEnv([make_env(n_links,n_obs,ep_len,use_her)])
    eval_callback = EvalCallback(eval_env, best_model_save_path=f'./results/{trial_name}/models',
                             log_path=f'./results/{trial_name}/models', eval_freq=5000,
                             deterministic=True, render=False)
    model.learn(total_timesteps=timesteps, log_interval=20, reset_num_timesteps=False, callback=eval_callback)
    model.save("./results/" + trial_name + '/2mil')