function [jrs] = load_jrs(q, qd)
    ...
    jrs_path = add_jrs_path();
    jrs_key_tmp = load(fullfile(jrs_path, 'c_kvi.mat'));
    ...
    for i = 1:length(qd)
        ...
        jrs_filename = sprintf('JRS_mat_%0.3f.mat', jrs_key(closest_idx));
        jrs_mat_load_tmp = load(fullfile(jrs_path, jrs_filename));
        jrs_mat_load = jrs_mat_load_tmp.JRS_mat;
        ...
        for jrs_idx = 1:length(jrs_mat_load)
            A = [cos(q(i)), -sin(q(i)), 0, 0, 0, 0; sin(q(i)), cos(q(i)), 0, 0, 0, 0; [zeros(4, 2), eye(4)]];
            jrs{jrs_idx, 1}{i, 1} = A*zonotope_slice(zonotope(jrs_mat_load{jrs_idx}), 5, qd(i));
        end
        ...
        for jrs_idx = 51:length(jrs_mat_load)
...
function jrs_path=add_jrs_path()
...
jrs_path = fullfile(rootFile,'zonopy','load_jrs','jrs_mat_saved');
addpath(jrs_path)
end