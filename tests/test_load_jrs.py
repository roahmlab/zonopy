...
gen = zpr.trajectory.PiecewiseArmTrajectory(q,
                                            dq,
                                            q,
                                            np.array(zp.polyZonotope([[0],[1]],1,id=0)),
                                            krange=param_range)
Q = gen.getReference(times)
Q = zp.remove_dependence_and_compress(Q[0][0], np.array(0))
Q = zp.cos_sin_cartprod(Q, 1)


q = torch.tensor([0, torch.pi])
dq = torch.tensor([torch.pi, torch.pi/2])
PZ_JRS,_ = zpr.load_JRS_trig(q,dq)
BPZ_JRS,_ = zpr.load_batch_JRS_trig(q, dq)
