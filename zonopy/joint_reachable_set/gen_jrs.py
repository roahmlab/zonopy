...
def gen_JRS(q,dq,joint_axes=None,taylor_degree=1,make_gens_independent=True):
    ...
    c_k = torch.zeros(n_q)
    g_k = torch.min(torch.max(PI/24,dq/3),PI/3)
    T, K = [],[]
    ...
    if traj_type == 'orig':
        for j in range(n_q):
            ...
            Qdd_brake.append((-1)*Qd_plan[-1]*(1/(T_full-T_plan))) # NOTE: - or (-1)* ??
...
if __name__ == '__main__':
    ...
    Q_des, Qd_des, Qdd_des, Q, Qd, Qd_a, Qdd_a, R_des, R_t_des, R, R_t = gen_JRS(q,dq,joint_axes=None,taylor_degree=1,make_gens_independent=False)
    ...
    zp.plot_JRSs(Q_des,deg=1,plot_freq=1,ax=ax)
