...
def gen_grad_RTS_2D_Layer(link_zonos,joint_axes,n_links,n_obs,params):
    ...
    class grad_RTS_2D_Layer(torch.autograd.Function):
        ...
        def forward(ctx,lambd,observation):
            ...
            for i in unsafe_flag.nonzero().reshape(-1):
                class nlp_setup():
                    ...
                    def objective(nlp,x):
                        qplan = ctx.qpos[i] + ctx.qvel[i]*T_PLAN + 0.5*x*T_PLAN**2
                    ...
                    def constraints(nlp,x): 
                        ...
                        if (nlp.x_prev!=x).any():      
                            ...
                            possible_max_min_q_dot = torch.vstack((ctx.qvel[i],ctx.qvel[i]+x*T_PLAN,torch.zeros_like(ctx.qvel[i])))
                ...
                if info['status'] == 0:
                    ctx.lambd[i] = torch.tensor(k_opt,dtype = torch.get_default_dtype())/g_ka
                    ctx.flags[i] = 0
                else:
                    ctx.flags[i] = 1
        ...
        def backward(ctx,*grad_ouput):
            ...
            grad_lambd = (grad_ouput[0]*(~strongly_active)).reshape(ctx.lambd_shape)
