...
def rtd_pass(A, b, FO_link, qpos, qvel, qgoal, n_timesteps, n_links, n_obs_in_frs, dimension, g_ka, ka_0, lambd_hat):
...
def gen_DART_2D_Layer(link_zonos, joint_axes, n_links, n_obs, params, num_processes=NUM_PROCESSES, dtype = torch.float, device=torch.device('cpu'), multi_process=False, gradient_step_sign = '-'):
                ...
                if n_qp > 0:
                    ...
                    try:
                        ...
                        for j,i in enumerate(qp_solve_ind):                            
                            ...
                            f_d = sp.csr_matrix((f_d_scaled[j], ([0] * n_links, range(n_links))))
                            qp = gp.Model("back_prop")
                            qp.Params.LogToConsole = 0
                            ...
                            try:
                                if gradient_step_sign == '-':
                                    grad_input[i] = - scale_factor_f_d[j,0] * torch.tensor(z.X.reshape(n_links),dtype=dtype,device=device)
                                else:
                                    grad_input[i] = scale_factor_f_d[j,0] * torch.tensor(z.X.reshape(n_links),dtype=dtype,device=device)
                            except:
...
if __name__ == '__main__':
    ...
    dart = gen_DART_2D_Layer(link_zonos, joint_axes, env.n_links, env.n_obs, params,device=device,dtype=dtype)
    ...
    for _ in range(n_steps):
        ...
        lam, FO_link, flag, nlp_info = dart(torch.vstack([lam_hat] * n_batch)+bias, observ_temp, None)
