    ...
    class grad_RTS_2D_Layer(torch.autograd.Function):
        ...
        def forward(ctx,lambd,observation):
            ...
            if num_problems > 0:
                ...
                results = pool.starmap(solve_rts_star,
                                       [(FO_link, As, bs) + x for x in
                                        zip([n_links] * num_problems,
                                            ...
                                            ctx.qpos[nonzero_flag_indices],
                                            ctx.qvel[nonzero_flag_indices],
                ...
                ctx.lambd[nonzero_flag_indices] = nonzero_indices_lambdas.to(ctx.lambd.dtype)
                ctx.flags[nonzero_flag_indices] = nonzero_indices_flags.to(ctx.flags.dtype)
...
if __name__ == '__main__':
    ...
    for _ in range(n_steps):
        ...
        bias1 = torch.full((batch_size,1),0.0,requires_grad=True )
        lam, FO_link, flag = RTS(torch.vstack([lam] * batch_size)+bias1,torch.vstack([observ_temp] * batch_size)+bias1)
        #lam.sum().backward()
